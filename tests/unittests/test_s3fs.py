...
class TestS3fsFuncs(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        environ['AWS_ACCESS_KEY_ID'] = self.access_key
        environ['AWS_SECRET_ACCESS_KEY'] = self.secret_key
    ...
    def test_gzip_csv_reads_writes_correctly(self) -> None:
        s3 = S3(use_keys = False, ssl = False)
        test_df = pd.read_csv(join(self.fixtures_folder, self.test_csv_name))
        test_df_rows = len(test_df.index)
        s3.to_csv(test_df, self.bucket_name, f'{self.test_csv_name}.gz', compression = 'gzip')
        s3_df = s3.read_csv(self.bucket_name, f'{self.test_csv_name}.gz', compression = 'gzip')
    ...
    def test_json_reads_writes_correctly(self) -> None:
        ...
        s3.to_json(test_dict, self.bucket_name, self.test_json_name)
        s3_dict = s3.read_json(self.bucket_name, self.test_json_name)
    ...
    def test_snappy_parquet_reads_writes_correctly(self) -> None:
        ...
        test_df_rows = len(test_df.index)
        s3.to_parquet(test_df, self.bucket_name, self.test_parquet_name)
        s3_df = s3.read_parquet(self.bucket_name, self.test_parquet_name)
        self.assertEqual(len(s3_df.index), test_df_rows)
    ...
    def test_object_exists(self) -> None:
        ...
        self.assertTrue(s3.obj_exists(self.bucket_name, self.test_csv_name))
